...
class WithdrawalPiiErasureIT {
    ...
    /** V14 의 소급 파기 문장. 마이그레이션 원문과 같아야 의미가 있다. */
    private static final String V14_ERASE_WITHDRAWN_PERSONAL_DATA = """
            UPDATE users
               SET email = NULL,
                   name = NULL,
                   nickname = NULL,
     ...
     *
     * <p>V14 의 UPDATE 문을 그대로 실행해 판정한다 — 마이그레이션은 이미 적용된 뒤라
     * 재실행으로는 효과를 볼 수 없기 때문이다. <b>활성 회원을 건드리지 않는 것</b>이
     * 이 문장에서 가장 위험한 부분이라 대조군을 함께 둔다.
     */
    @Test
    ...
    void backfillErasesOnlyWithdrawnRows() {
        ...
        jdbcTemplate.update(V14_ERASE_WITHDRAWN_PERSONAL_DATA);
