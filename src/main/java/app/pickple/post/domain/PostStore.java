...
public interface PostStore {
    ...
    /**
     * 상세 화면 한 건 (§6.2·§6.3). 응답 계약은 ADR-0046 이 정한다.
     *
     ...
     *                      일반 게시글은 0 이다
     * @param commentCount  댓글 <b>건수</b>. 화면 표시용이라 인기순 점수와 다르다 (R-24·R-25)
     * @param authorNickname 닉네임, 없으면 이름, 둘 다 없으면 {@code "알 수 없음"} —
     *                      탈퇴로 파기된 작성자다 (ADR-0040). 목록과 같은 표기다
     * @param authorRanking 작성자의 TOP 피커 순위. 배치가 매기기 전이거나 탈퇴한 회원이면
     *                      {@code null} 이다 — 0 을 지어내지 않는다 (ADR-0028)
     * @param authorGrade   작성자가 도달한 최고 등급 (R-16). 가입 시 LV.1 이 기본이라 비지 않고,
     *                      활동의 결과라 탈퇴해도 파기되지 않는다 (R-20)
     * @param myOptionId    내가 고른 선택지. <b>{@code null} 이면 아직 투표하지 않았다</b>
     *                      (게스트 포함). 이 값의 유무가 득표율 노출을 가른다 (ADR-0046)
     */
     ...
     * 미투표자에게 감추는 판단은 서비스가 한다 (ADR-0046) — 감춤이 화면 계약이지
