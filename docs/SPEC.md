...
- **목록의 인기순 키 조회를 재사용한다.** `post`의 인기순 인덱스로 삭제되지 않은 상위 10개 id를
  ...
  `post.id IN (...)`에 해당하는 상품별 대표 사진을 한 번의 배치 조회로 가져온다.
  비어 있지 않은 결과는 키 → 행 → 상품의 세 SQL 문장이며, REPEATABLE READ 한 트랜잭션으로 묶는다.
  상품·사진을 정렬·LIMIT 전에 조인하거나 게시글마다 따로 조회하지 않는다.
  인기 경로의 세 SQL에는 서브쿼리가 없다. 상품·사진을 `LEFT JOIN`으로 읽고 사진 id 순서에서
