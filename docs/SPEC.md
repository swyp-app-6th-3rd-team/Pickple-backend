...
- QA 로그인 시도에 서버 전체가 공유하는 횟수 제한을 두지 않는다. 반복 실패 후에도 올바른
  자격증명과 사용자 상태를 검증하고 로그인할 수 있다.
...
| 날짜 | 변경 | 계기 |
|---|---|---|
| 2026-09-22 | 독립 QA 계정 발급과 DB 자격증명 로그인, 설정 파일 기반 전체 프로필 활성화, V18·탈퇴 정리. QA 환경변수와 전체 공유 시도 제한 없음 | Issue #185. 소셜 계정 없이 심사·테스터별 계정을 제공하고 로그인 간 차단 방지 |
