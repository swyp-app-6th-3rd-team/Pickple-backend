...
- 기존 랜덤 기본 이미지 선택과 이미지 생략 시 현재 이미지 유지 규칙을 사용한다.
- `PROFILE_DEFAULT_IMAGE_URLS`를 지정하면 해당 목록에서 고른다. 생략하면 기존
...
- 이미지 객체를 먼저 배치하고 모든 후보의 익명 GET·PNG 디코딩을 확인한 뒤 앱을 배포한다.
  운영 적용 순서와 복구 한계는 `terraform/README.md`의 기본 프로필 이미지 항목을 따른다.
...
- F03 프론트 요청에 따라 `GET /posts`의 `PostListItem`에 `products[{displayOrder, imageUrl}]`를 추가한다.
  `LATEST`·`POPULAR`와 카테고리 유무에 공통 적용하며 `thumbnailUrl` 및 나머지 기존 필드는 유지한다.
  - `A_B`: 표시 순서대로 두 상품. `displayOrder=1`은 A(왼쪽), `2`는 B(오른쪽)다.
  - `AGREE`: 표시 순서 1인 상품 하나와 최초 등록 사진. `GENERAL`: `products=[]`.
  - 사진이 없으면 상품·표시 순서는 유지하고 `imageUrl=null`을 반환한다. A 사진이 없을 때 B로 대체하지 않는다.
  - 정렬·커서로 페이지를 먼저 확정한 뒤, 해당 페이지의 투표 게시글에만 인기 카드의 상품 사진 배치 조회를 재사용한다.
    상품별 `item_resource.id` 오름차순 첫 사진을 사용하고, 같은 결과로 기존 `thumbnailUrl`을 채운다.
    빈 페이지는 1문장, 일반 게시글만 있으면 2문장, 투표 게시글이 있으면 3문장이다.
    상품 조회는 `LEFT JOIN`이며 서브쿼리가 없고, 다음 페이지 확인용 초과 1건은 상품 조회에 포함하지 않는다.
  - 기능명세서 v0.4 §4.2의 A 사진 한 장 표기는 원문으로 보존한다. 두 상품 사진은 이번 프론트 요청에 따른 계약 확장이다.
...
- **작성자 등급**은 `authorGradeLevel`(1~5)·`authorGradeName`(`LV.1`~`LV.5`)으로 제공한다(#176·F04).
  상세와 같은 `users.highest_grade` 저장값과 `Grade`를 사용하며 포인트로 재계산하지 않는다.
  조회 시점의 저장 등급이므로 승급 후 재조회에 반영된다. 탈퇴 시에도 유지되어 두 필드는 null이 아니다.
  작성자 이름·닉네임이 파기되면 기존처럼 `알 수 없음`을 표시한다. 작성자 행은 외래 키와 탈퇴 정책상 남는다.
  기존 작성자 조인에서 함께 읽어 비어 있지 않은 목록의 키·행 2문장을 유지한다.
...
- **목록의 인기순 키 조회를 재사용한다.** `post`의 인기순 인덱스로 삭제되지 않은 상위 10개 id를
  먼저 확정한다. 그 id에만 작성자·기존 목록 필드·댓글 작성자 수를 붙이고,
  그중 찬반·A/B의 `post.id IN (...)`에 해당하는 상품별 대표 사진을 한 번의 배치 조회로 가져온다.
  빈 결과는 키 조회 1문장, 일반 게시글만 있으면 키 → 행 2문장, 찬반·A/B가 포함되면
  키 → 행 → 상품 3문장이다. 여러 문장은 REPEATABLE READ 한 트랜잭션으로 묶는다.
  상품·사진을 정렬·LIMIT 전에 조인하거나 게시글마다 따로 조회하지 않는다.
  인기 경로의 SQL에는 서브쿼리가 없다. 상품·사진을 `LEFT JOIN`으로 읽고 사진 id 순서에서
  상품별 첫 행을 선택한다. 기존 `thumbnailUrl`도 이 배치 결과의 표시 순서 1 상품 사진으로 채운다.
  찬반 사진이 최대 3장인 만큼 사진 후보 행을 더 읽는 대신 상품별 스칼라 서브쿼리와 썸네일 중복 조회를 없앤다.
- **파라미터가 없다.** 카테고리 필터도 커서도 크기 조절도 받지 않는다. 홈 화면의 계약은
  "전체에서 인기 상위 열 건" 하나뿐이고, 조절 손잡이를 열면 목록 API 와 구별되지 않는다.
- **응답은 커서 봉투 없이 인기 카드 전용 `PopularPostItem` 배열이다.**
  기존 `PostListItem`의 필드는 유지하고 다음 두 필드를 추가한다. `GET /posts`의 상품 사진 확장은 F03 계약(§3.3)으로 별도 정의한다.
  - `products: [{ displayOrder, imageUrl }]`: 상품별로 가장 먼저 등록한 사진 한 장.
    A/B는 `displayOrder=1(A), 2(B)` 순서의 두 건, 찬반은 `displayOrder=1` 한 건, 일반은 빈 배열이다.
    OpenAPI 상품 스키마는 상세의 `ProductItem`과 구분하는 `PopularProductItem`이다.
  - `commenterCount`: `post.commenter_count`의 서로 다른 댓글 작성자 수(R-25).
    기존 누적 인원 정책에 따라 댓글 삭제 시에도 감소하지 않는다.
    `commentCount`는 댓글 건수를 유지하므로 같은 사람이 댓글을 5개 쓰면 각각 1과 5다.
  `thumbnailUrl`은 클라이언트 호환용으로 유지한다. 찬반은 첫 사진, A/B는 A 상품 사진, 일반은 null이다.
  `nextCursor`·`hasNext` 는 싣지 않는다 — Top 10 은 그 열 건이 전부인데
...
기본 응답 구조·결과 공개 조건은 [ADR-0046](adr/0046-post-detail-single-type-with-nested-vote-section.md),
상세 상품의 전체 사진 추가 계약은 Issue #181을 따른다.
...
- **상세 상품의 전체 사진은 `vote.products[].imageUrls`로 제공한다** (Issue #181).
  해당 상품 컨테이너의 사진 URL 전체를 `item_resource.id ASC`로 반환하며 항상 배열이다.
  정상 찬반은 상품 1개에 사진 1~3장, A/B는 상품 2개에 각각 1장이다. 상품 순서는 `displayOrder ASC`다.
  기존 `imageUrl`은 배열 첫 값을 유지한다. 사진이 없는 과거/비정상 데이터도 상품을 남기고
  `imageUrl: null`, `imageUrls: []`로 반환하며 다른 상품 사진으로 대체하지 않는다.
  - 기능명세서 v0.5는 등록 1~3장과 상세 첫 사진 1장을 구분한다(구현 전 검토서 기준).
    이번 요청은 상세 조회 계약 확장이다. 원문을 수정하거나 기존 전체 사진 요구의 누락으로 해석하지 않는다.
  - 게스트·미투표자·투표자의 사진 제공 범위는 같다. 목록·인기·랜덤·검색·활동의 대표 사진 계약,
    등록 장수 제한과 소유권·용도·재사용 방지 정책은 그대로다.
  - 프론트 상세 갤러리는 `imageUrls`를 사용한다. 기존 `imageUrl`만 사용하면 첫 사진만 표시된다.
- **읽기는 고정된 세 문장이다** — ①게시글+작성자+내 투표 ②상품+전체 사진 ③선택지. QueryDSL
  ...
  선언하고 저장소가 진입 시점에 단언한다. 상품·사진 LEFT JOIN 행은 상품 ID로 묶고 선택지는 별도로 읽는다.
  정상 찬반의 사진 3행은 상품 1개로 조립하며 사진별·상품별 추가 SQL은 없다.
  없는/삭제된 게시글은 본문 1문장에서 끝난다. 기존 저장 구조를 이용하므로 마이그레이션은 필요하지 않다.
...
- 각 댓글의 `authorGradeLevel`(1~5)·`authorGradeName`(`LV.1`~`LV.5`)은 게시글 목록·상세와 같은
  현재 `users.highest_grade` 저장값이다(#176·F04). 기존 작성자 조인에서 읽으므로 추가 조회가 없다.
  가입 기본값은 1·`LV.1`이며 탈퇴 후에도 저장 등급을 유지해 두 필드는 null이 아니다.
  탈퇴로 이름·닉네임이 파기되면 `nickname`은 기존처럼 `알 수 없음`이다.
...
- **미션은 누적(`TOTAL_VOTE`)·연속(`STREAK_VOTE`) 순서로 계열마다 하나씩**이다.
  각각 **아직 못 넘은 가장 낮은 임계값**을 고른다 — "하위 미션 먼저 표시" 다.
  다 채운 계열은 빠지고, 누적·연속 계열을 모두 채우면 일일 뱃지 보유 여부와 무관하게 빈 배열이다.
  원본 기능명세 §2.3의 누적·일일 슬롯에서 **2026-09-20 기획 합의로 홈 UI에 맞춰 변경**했다.
  일일(`DAILY_VOTE`) 계열은 이 미션 응답에서만 임시 제외하며, 획득 판정과 전체 뱃지 목록은 유지한다.
...
| GET | `/users/me/posts/recent` | 필요 | 7일 이내 올린 본인 투표의 상품 사진과 선택지별 결과 |
...
- **댓글 활동 항목만 대표 댓글과 그 원픽 수를 더 싣는다**(#158). `/comments` 응답 항목에
  공통 10필드에 더해 `myComment`(원문 — 1줄 줄임은 클라이언트 몫)와 `myCommentOnePickCount` 가 붙는다.
  - **대표는 원픽이 가장 많은 내 댓글 1건, 동률이면 최신**이다(기획 확정 2026-09-12).
    항목 단위는 게시글 그대로라 커서 `(첫 댓글 시각, postId)`·정렬·ADR-0036·ADR-0043 전제가 그대로 유효하다.
  - **원픽 수는 대표 댓글의 것이다.** 그 글에서 내 댓글들이 받은 **합계가 아니다** —
    대표가 아닌 내 댓글이 받은 원픽은 이 숫자에 들어가지 않는다.
  - 대표 선별은 SQL 이 아니라 애플리케이션이 한다. 게시글별 1위를 SQL 로 고르려면 윈도 함수를
    쓰거나 게시글마다 상관 서브쿼리를 돌려야 하는데 후자는 **문장 수에 잡히지 않는 N+1** 이다.
    조각 전체의 내 댓글을 `post_id IN` **한 문장**으로 읽어 자바가 고른다 — 댓글 경로는 세 문장이다.
  - 동률 판정에 `id` 가 함께 쓰인다. `created_at` 이 `datetime(0)` 이라 같은 초에 쓴 두 댓글이
    시각만으로는 갈리지 않는다.
- **내 댓글을 전부 삭제한 글은 목록에 나오지 않는다**(#158). `post_commenter` 는 참여 원장이라
  댓글을 지워도 행이 남는다 — `PostCommenterStore` 에 삭제 경로가 없고 `CommentService.delete` 도
  건수만 줄이고 인원은 그대로 둔다. 그 append-only 성질은 일부러 남긴 것이다:
  `created_at` 이 "이 사람의 **첫** 댓글 시각" 이고 그 값이 곧 정렬 키이자 커서라(R-32),
  행을 지우면 커서가 기준을 잃는다. 그래서 원장을 고치는 대신 **살아있는 내 댓글이 1건 이상**
  이라는 조건을 목록에 더한다. 일부만 지워 살아있는 댓글이 남은 글은 그대로 나오고,
  대표는 **살아있는 댓글 중에서** 뽑는다.
- **요약의 `commentCount` 도 같은 조건을 쓴다**(#158). 목록에만 걸면 칩 숫자와 카드 장수가
  어긋난다 — 사용자가 보는 것은 행이 아니라 카드다. R-25 의 "한 글에 여러 개 달아도 1" 은
  그대로이되, **살아있는 내 댓글이 0 이면 세지 않는다**.
...
- **최근 투표 카드는 전용 `RecentVotePostItem` 으로 본인이 작성한 투표의 결과를 싣는다**(#159·B06).
  `/users/me/posts/recent` 는 작성 시각 내림차순, 동률이면 게시글 ID 내림차순으로 최신 10건을 반환한다.
  기존 공통 10필드(`id`, `type`, `category`, `title`, `description`, `commentCount`, `voteCount`,
  `thumbnailUrl`, `createdAt`, `activityAt`)를 유지하고 `products` 와 `options` 를 추가한다.
  최상위 **`voteCount` 는 총 투표 수**이며 `activityAt` 은 작성 시각이다. 기존 `thumbnailUrl` 도 유지한다.
  - `products[{ displayOrder, imageUrl }]`: 찬반은 대표 상품 사진 1장, A/B는 표시 순서 1·2의 상품 사진 2장이다.
  - `options[{ optionId, label, displayOrder, voteCount, percentage }]`: 표시 순서대로 선택지 2개를 반환한다.
    `label` 은 찬반 선택지의 라벨이고 A/B에서는 null이다. 선택지의 `voteCount` 는 해당 선택지의 득표 수다.
  - **작성자가 직접 투표하지 않았어도 선택지별 득표 수·득표율을 반환한다.** 이 예외는 인증된 본인의
    최근 투표 카드에만 적용한다([ADR-0051](adr/0051-recent-vote-card-author-results.md)). 상세·랜덤 카드의
    미투표자 집계 감춤 정책과 다른 활동 목록의 계약은 유지한다. 작성 카드이므로 `selectedOptionId` 는 추가하지 않는다.
...
- **투표 활동 항목은 내 선택과 선택지·상품을 더 싣는다**(#157). `/votes` 응답 항목에 공통 10필드에 더해
...
- **득표율은 상세와 같은 정본(`VotePercentage`)으로 계산한다.** 같은 글을 상세로 열든
  투표 활동 목록이나 최근 투표 카드로 보든 게이지 값이 같아야 하므로 정수 반올림 규칙을 공유한다.
  총 투표 수가 0이면 각 선택지의 득표율도 0이다. 선택지별로 반올림하므로 **두 값의 합이 100 이 아닐 수 있다.**
...
- **이 경로에는 미투표자가 없다.** 상세는 미투표자에게 `voteCount`·`percentage` 를 감추지만
  ...
  감출 대상이 존재하지 않아 두 필드가 항상 있다. 최근 투표 카드의 작성자 예외는 ADR-0051 의 별도 계약이다.
- **선택지·상품은 조각 전체에 한 문장씩 붙는다.** 행 문장에 조인하면 선택지 둘과 상품 둘이
  서로를 곱해 게시글 한 줄이 넉 줄로 불어나고, 게시글마다 따로 물으면 N+1 이다.
  확정된 id 를 `post.id IN (…)` 으로 읽어 **투표 활동과 최근 투표 카드 경로는 비어 있지 않으면 네 문장**이다
  (키 확정·기본 행·상품·선택지). 투표 활동은 조각 크기 10·50, 최근 카드는 1·10건에서 문장 수가 같다.
  상품 사진은 확정된 게시글의 상품·사진을 LEFT JOIN으로 읽고 상품별 가장 작은 사진 ID를 선택한다.
  최근 카드의 `thumbnailUrl` 도 이 결과의 첫 상품 사진을 사용하므로 최근 카드 조회에는 서브쿼리가 없다.
  사진이 없는 상품은 `imageUrl: null` 로 남으며 A 사진이 없다고 B 사진을 대표 사진으로 쓰지 않는다.
  SQL 문장 수가 일정하다는 것이 전체 지연시간까지 같다는 뜻은 아니다.
...
| 2026-09-12 | 댓글 활동 항목에 `myComment`·`myCommentOnePickCount` 추가(§3.10). 대표는 원픽 최다·동률이면 최신인 내 댓글 1건이고 원픽 수는 합계가 아니라 그 한 건의 것이다. **살아있는 내 댓글이 없는 글은 목록과 요약 `commentCount` 양쪽에서 빠진다** | Issue #158. 명세 v0.4 §9.2 가 댓글 활동에 "내가 남긴 댓글, 받은 원픽 갯수" 를 요구하는데 목록이 `post_commenter` 만 읽어 댓글 내용이 경로에 없었다. 기획이 대표 규칙과 노출 조건을 확정(2026-09-12)하면서 착수가 풀렸다. `post_commenter` 는 참여 원장이라 삭제 경로가 없고 `created_at` 이 커서 기준(R-32)이라 지울 수 없다 — 원장을 고치는 대신 "살아있는 내 댓글 1건 이상" 조건을 더했고, 목록에만 걸면 칩 숫자와 카드 장수가 어긋나 요약도 같은 조건을 받는다. 대표 선별은 상관 서브쿼리(문장 수에 안 잡히는 N+1)를 피해 `post_id IN` 한 문장으로 읽어 애플리케이션이 고른다. 기획이 정한 규칙의 구현이라 되돌리기 비싼 새 결정이 없어 ADR 을 만들지 않았다 |
...
| 2026-09-16 | 상세 상품에 전체 사진 배열 `imageUrls` 추가, 기존 `imageUrl` 유지(§3.3) | Issue #181. v0.5의 상세 첫 사진 계약을 사용자 요청으로 확장. resource ID 순서·빈 배열·A/B 분리 및 투표형 3문장 유지, 상품·사진 LEFT JOIN 후 상품 ID로 조립. 등록 정책과 다른 조회의 대표 사진 계약은 유지 |
| 2026-09-20 | 미해제 미션 응답을 `TOTAL_VOTE`·`STREAK_VOTE` 순서로 제한(§3.9) | Issue #183. 기획팀과 합의한 홈 UI 임시 정책. `DAILY_VOTE`는 미션 응답에서만 제외하고 획득 판정·전체 뱃지 목록 유지 |
