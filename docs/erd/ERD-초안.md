...
| 2 | 닉네임 대소문자 | **결정됨** — 구분하지 않는다 (#16). `Pick` = `pick` | `JpaUserStoreNicknameIT.collationIsCaseInsensitive()` 가 고정 |
...
| 7 | 탈퇴 상태 | **결정됨** — 정본은 `users.state` (#16, V5) | 아래 참고 |

7번은 이 초안과 #12를 겹쳐야 보이는 문제라 따로 적어둔다. **#16에서 `state`로 정했다** —
경위는 아래 문단 뒤에 이어 적는다.
...
**결정 (#16, `V5__active_nickname_follows_state.sql`) — `users.state`를 정본으로 삼는다.**

생성 컬럼을 `CASE WHEN state = 'ACTIVE' THEN nickname END`로 바꾸고 `deleted_at`을 걷어냈다.
근거는 "한 사실을 두 컬럼이 표현하는 구조 자체가 원인"이라는 것이다 — 어느 쪽을 남기든
둘을 함께 두면 다시 어긋난다. 코드가 이미 `state` 하나만 쓰고 있었으므로
이쪽으로 맞추는 편이 고칠 자리가 적고, 잊고 빠뜨릴 경로도 없다.

포기한 것은 **탈퇴 시각**이다. 지금은 `updated_at`으로 근사할 수 있고, 감사 목적으로
정확한 시각이 필요해지면 `users`에 컬럼을 되돌리는 대신 별도 이력 테이블을 만든다 —
그래야 "활성 여부"의 정본이 다시 둘로 갈라지지 않는다.

`JpaUserStoreNicknameIT.generatedColumnFollowsState()`가 이 결정을 고정한다.
