...
### 2.2 정책 판단은 저장소가 하지 않는다

처음엔 `JpaOnePickStore`·`JpaPointHistoryStore` 가 유니크 위반을 잡아
`DuplicatePickException` 을 던졌다. **경계 위반이다** — "이미 지급됐다" 는 정책 해석이고
저장소는 그 판단을 할 위치가 아니다. 서비스 계층을 만들어 걷어냈다(ADR-0019).

- 저장소: `Optional<Long> saveIfAbsent(...)` — 삽입됐는지 사실만
- 단일 애그리거트 규칙: 도메인 객체 (R-07)
- **여러 애그리거트에 걸친 규칙: 서비스** — R-12(원픽 1건 → 두 사람 지급),
  R-13(재지급 금지)은 댓글·원픽·포인트 세 도메인이 얽힌다

자세한 기준과 구현 중 만난 함정은 [계층별 책임](../domain/계층별%20책임.md) 참조.
...
| 2 | ~~정책 판단 위치~~ | **해결** — 서비스 계층 도입(ADR-0019) | — |
...
| 9 | 기존 상품·선택지 **수정·삭제** | 지원하지 않음 — 신규 추가만 반영 | 투표 시작 후 상품 변경 정책이 미정 |
| 10 | 삭제-상호작용 경합 | `ActivePostGuard` 확인 후 삭제가 끼어들면 통과 | 창이 좁고 삭제가 드물어 감수. 필요하면 잠금 |
...
| `OnePickServiceIT` | R-12 두 사람 지급, 중복 픽 정책 해석, 자기 댓글 차단 |
| `PostEditWithVoteIT` | 투표가 달린 게시글 수정 |
| `DeletedPostInteractionIT` | 삭제된 게시글 상호작용 차단 |
