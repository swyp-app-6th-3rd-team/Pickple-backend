...
**인기순 커서는 `Long` 으로 바인딩한다.** 컬럼은 `INT UNSIGNED` → `Integer` 매핑이라 타입이 어긋나는데, Hibernate 는
파라미터를 좌변 속성 타입으로 강제(coerce)하고 SQL 에 `cast` 를 내리지 않는다 — #133 의 인기순 경로가 같은 조합이다.
다만 이 사실은 컴파일러가 지켜주지 못하므로(`PostListCursor.sortValue` 가 `Object`) **실행계획 테스트가 캡처한 SQL 에
`cast(` 가 없음을 고정**한다. 좌변에 `cast` 가 붙는 순간 `idx_post_popular_all` 이 범위로 접히지 않는다.
프로젝션의 `longValue()` 는 행 문장에만 있고 정렬·커서에는 쓰지 않는다(ADR-0043 과 같은 규칙).
...
- 호출자가 트랜잭션을 열어야 한다는 전제가 생겼다. `JpaPostStore.findSlice` 가 `REPEATABLE_READ` 로 열고 저장소가
  단언한다. 인기 점수는 투표·댓글이 실시간으로 올리는 값이라 활동 시각보다 자주 바뀐다 — 스냅샷 전제는 순서뿐 아니라
...
| **인기순 커서를 `Integer` 로 복원**해 컬럼 타입과 맞추기 | 캡처한 SQL 에 `cast` 가 없어 필요가 없었다. `PostListCursorTest` 의 계약(`Long`)을 바꾸는 비용만 든다. `cast` 가 나타나면 이 안으로 간다 — 테스트가 그 순간을 잡는다 |
