...
- 액세스 토큰이 stateless 라 **즉시 무효화가 불가능하다.** 로그아웃해도 이미 발급된
  액세스 토큰은 만료(30분)까지 유효하다.
  > ⚠️ **이 판단은 로그아웃을 전제로 한다.** 회원 탈퇴에서는 이 한계가 실제 데이터 오염으로
  > 이어지는 것이 실측됐다(탈퇴자가 댓글·투표·원픽을 생성하고 포인트 원장에 적립).
  > 재검토는 [ADR-0035](0035-withdrawn-user-central-authorization.md) 초안 참조. 즉시 차단이 필요하면 블랙리스트(Redis 등)가 필요한데,
