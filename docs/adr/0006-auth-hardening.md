...
추가로 refresh token 회전은 제출된 해시를 조건으로 한 CAS 갱신으로 구현한다. 같은 token으로
동시 요청이 와도 한 요청만 새 해시를 저장하며, 경합에서 진 요청과 이미 회전된 token은 401로 거부한다.
이때 현재 저장된 승자 token은 폐기하지 않는다. 사용자당 한 행인 현재 구조로는 악의적 재사용과
정상적인 지연·재시도 요청을 구분할 수 없기 때문이다. 구분이 필요하면 token-family 스키마가 필요하다.

## 결과

**검증한 것** (통합 테스트 + 실 HTTP 호출)

| 항목 | 결과 |
|---|---|
| 인증 없이 보호 API | 401 + `ApiResponse` JSON, `WWW-Authenticate` 헤더 없음 |
| 토큰 회전 | 재발급 시 쿠키 값이 바뀌고 DB 행은 누적되지 않음 |
| 회전된 옛 토큰 재사용 | 401, 현재 저장된 승자 token은 계속 유효 |
