...
- **`.env.example` 이 문서이자 스키마가 된다.** 주석을 지우다 마커를 함께 지우면 키가 조용히 로컬 전용이 될 수 있다. 마커 개수와 파싱된 키 개수 대조만으로는 이걸 못 잡는다 — 마커를 지우면 양쪽이 똑같이 줄기 때문이다. 그래서 `--check` 는 세 가지를 더 본다: `terraform output` 과의 대조, compose 가 쓰는 변수의 공급 여부, 그리고 로컬 전용이어야 할 키(`spring_profiles_active` 등)에 `@secret` 이 붙었는지.
- **중복 선언은 정책을 조용히 무력화한다.** 같은 키를 두 번 선언하면 첫 레코드의 정책만 적용되어 `@remote-wins` 같은 보호가 사라진다. `--check` 가 중복을 검출한다.
