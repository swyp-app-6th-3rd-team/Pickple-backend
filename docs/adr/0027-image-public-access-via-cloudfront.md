...
**감수한 위험 — 삭제 복구 경로가 없다.** 버킷 버저닝을 켜지 않았다. 런타임 역할에
`s3:DeleteObject` 가 있으므로(보상 삭제에 필요하다), 앱 버그나 인스턴스 자격증명 탈취로
객체가 지워지면 **복구할 방법이 없다.** 버저닝을 켜면 삭제가 삭제 마커만 남겨 되살릴 수
있지만, 모든 버전이 과금 대상이 되고 수명주기 규칙을 따로 관리해야 한다.
6주 MVP 이고 이미지가 유실돼도 사용자가 다시 올릴 수 있는 성격이라 감수한다.
**수명이 긴 환경에 이 구성을 재사용한다면 버저닝을 먼저 켜야 한다.**
