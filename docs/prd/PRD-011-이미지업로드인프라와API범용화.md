...
> **완료 선언 조건**: develop 에 배포한 뒤 **실제 API 를 호출해 업로드가 되는 것을 확인하기 전에는
> 이 작업을 완료로 선언하지 않는다.** 3~8번은 배포 후에만 채워지며, 그 전까지 이 PRD 는 "진행 중"이다.
> `/actuator/health` 는 근거가 되지 않는다 — 버킷 미설정 예외는 요청 시점에 나므로
> 설정이 틀려도 헬스체크는 계속 초록이다.
